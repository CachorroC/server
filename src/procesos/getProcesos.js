...
exports.getProcesos = exports.fetchProcesos = exports.llaves = void 0;
...
const rows = [];
const failed = [];
async function fetchProcesos(llaveProceso) {
    ...
    if (dataRaw.procesos.length === 0) {
        failed.push(llaveProceso);
        const failedString = JSON.stringify(failed);
        fs.writeFile("consultas_canceladas.json", failedString);
    }
    else {
        rows.push(dataRaw.procesos);
        console.log("rows length: " + rows.length);
        const procesosString = JSON.stringify(dataRaw);
        console.log("procesosRaw: " + procesosString);
        const rowsFlat = rows.flat(1);
        const rowsString = JSON.stringify(rowsFlat);
        fs.writeFile("ConsultasNumeroRadicacion.json", rowsString);
    }
    return dataRaw;
}
exports.fetchProcesos = fetchProcesos;
async function getProcesos() {
    const res = exports.llaves.forEach((llave, index) => {
        console.log("llave:" + llave);
        console.log("index:" + index);
        setTimeout(() => {
            console.log("llave length:" + llave.length);
            fetchProcesos(llave);
        }, index * 1000);
    });
}
